...
IMDB_TEMPLATE = environ.get("IMDB_TEMPLATE","<b>Query: {query}</b> \n‌IMDb Data:\n───────   • ◆ •   ───────\n\n⍞  𝗧𝗶𝘁𝗹𝗲 : <code><a href={url}>{title}</code>\n★ 𝗥𝗮𝘁𝗶𝗻𝗴 : <code><a href={url}/ratings>{rating}</code> / 10\n⎚ 𝗩𝗼𝘁𝗲𝘀 : <code>{votes}</code>\n⌗ 𝗚𝗲𝗻𝗿𝗲𝘀 : <code>{genres}</code>\n⌥ 𝗥𝘂𝗻𝘁𝗶𝗺𝗲 : <code>{runtime} Minutes</code>\n⌬ 𝗟𝗮𝗻𝗴𝘂𝗮𝗴𝗲𝘀 : <code>{languages}</code>\n〄 𝗥𝗲𝗹𝗲𝗮𝘀𝗲𝗱 : <code><a href={url}/releaseinfo>{year}</code>\n߷ 𝐔𝐩𝐥𝐨𝐚𝐝𝐞𝐝 𝐁𝐲 : ‣ <code>{message.from_user.mention}</code>\n\n\n〠 𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗕𝘆 :- {query.message.chat.title}</i>\n╰───────   • ◆ •   ───────╯")
