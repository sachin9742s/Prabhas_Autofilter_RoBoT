...
IMDB_TEMPLATE = environ.get("IMDB_TEMPLATE","<b>Query: {query}</b> \n‌IMDb Data:\n\n⍞  𝗧𝗶𝘁𝗹𝗲 : <a href={url}>{title}</b>\n★ 𝗥𝗮𝘁𝗶𝗻𝗴 : <a href={url}/ratings>{rating}</b> / 10\n⎚ 𝗩𝗼𝘁𝗲𝘀 : {votes}</b>\n⌗ 𝗚𝗲𝗻𝗿𝗲𝘀 : {genres}</b>\n⌥ 𝗥𝘂𝗻𝘁𝗶𝗺𝗲 : {runtime}</b> Minutes\n⌬ 𝗟𝗮𝗻𝗴𝘂𝗮𝗴𝗲𝘀 : <code>{languages}</code>\n〄 𝗥𝗲𝗹𝗲𝗮𝘀𝗲𝗱 : <a href={url}/releaseinfo>{year}</b>\n\n\n𝐔𝐩𝐥𝐨𝐚𝐝𝐞𝐝 𝐁𝐲 : ‣ {message.from_user.mention}</b>")
